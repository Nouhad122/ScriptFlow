...
function AnalyticsCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
...
function AnalyticsSection({ analytics }: { analytics: PipelineAnalytics }) {
  ...
  return (
    <div className="grid grid-cols-4 gap-3">
      <AnalyticsCard label="Total Runs" value={String(analytics.totalRuns)}
        sub={`${analytics.completedRuns} completed · ${analytics.failedRuns} failed`} />
      <AnalyticsCard label="Success Rate"
        value={analytics.totalRuns > 0 ? `${analytics.successRate}%` : '—'}
        sub={analytics.totalRuns === 0 ? 'No runs yet' : undefined} />
      <AnalyticsCard label="Avg Execution" value={formatDuration(analytics.averageTotalMs)}
        sub={analytics.averageTotalMs === null ? 'No timing data yet' : undefined} />
      <AnalyticsCard label="Longest Run" value={formatDuration(analytics.longestTotalMs)}
        sub={analytics.fastestTotalMs !== null ? `Fastest: ${formatDuration(analytics.fastestTotalMs)}` : undefined} />
...
function buildTimeline(run: PipelineRunRecord): TimelineStep[] {
  ...
  const failedIdx  = run.failedStage ? STAGE_ORDER.findIndex(s => s.key === run.failedStage) : -1
  ...
  const stageTiming = (ms: number | null): string | null =>
    !hasTimings || ms === null ? null : formatDuration(ms)

  return [
    { label: 'Pipeline Started',  status: 'completed',        timing: formatDateTime(run.startedAt) },
    { label: 'Idea Generation',   status: stageStatus(0),     timing: stageTiming(run.ideaGenerationMs),
      note: failedIdx === 0 ? (run.errorMessage ?? undefined) : undefined },
    { label: 'ICE Scoring',       status: stageStatus(1),     timing: stageTiming(run.iceScoringMs),
      note: failedIdx === 1 ? (run.errorMessage ?? undefined) : undefined },
    { label: 'Persistence',       status: stageStatus(2),     timing: stageTiming(run.persistenceMs),
...
function TimelineStepRow({ step, isLast }: { step: TimelineStep; isLast: boolean }) {
  ...
  const IconComp =
    ...
    step.status === 'failed'    ? XCircle       : null
  ...
  return (
    <div className="flex gap-3">
      ...
      <div className="flex flex-col items-center">
        <div className={cn('flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2', dotClass)}>
          {IconComp && <IconComp className={cn('h-2.5 w-2.5', iconClass)} />}
        </div>
        {!isLast && (
          <m.div
      ...
      <div className={cn('min-w-0 flex-1', isLast ? 'pb-0' : 'pb-5')}>
        <div className="flex items-baseline justify-between gap-2">
          <p className={cn('text-sm font-medium', step.status === 'skipped' ? 'text-muted-foreground' : 'text-foreground')}>
            {step.label}
          </p>
          {step.timing && (
  ...
  return (
    <>
      ...
      <m.div
        ...
        animate="visible"
        exit="exit"
        className="fixed inset-y-0 right-0 z-50 flex w-125 flex-col border-l border-border bg-background shadow-2xl"
      >
        {/* Drawer header */}
          ...
          <div className="px-6 py-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-lg border border-border bg-card px-4 py-3">
                ...
                <p className="mt-0.5 text-xl font-bold text-foreground">{formatDuration(run.totalMs)}</p>
              </div>
            </div>
            ...
            <div className="mt-3 rounded-lg border border-border bg-card">
              ...
              <div className="divide-y divide-border">
                <div className="flex items-center justify-between px-4 py-2.5">
                  ...
                  <span className="text-sm font-semibold text-foreground">{run.approvedCandidates}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5">
                  <span className="text-sm text-yellow-400">Consider</span>
                  <span className="text-sm font-semibold text-foreground">{run.considerCandidates}</span>
                </div>
                <div className="flex items-center justify-between px-4 py-2.5">
                  <span className="text-sm text-red-400">Reject</span>
                  <span className="text-sm font-semibold text-foreground">{run.rejectedCandidates}</span>
                </div>
              </div>
            </div>
            ...
            {run.status === 'failed' && run.failedStage && (
              <div className="mt-3 rounded-lg border border-red-500/20 bg-red-500/5 px-4 py-3">
                <p className="text-xs font-semibold text-red-400">Failed at: {run.failedStage}</p>
                {run.errorMessage && (
                  <p className="mt-1 text-xs leading-relaxed text-red-400/70">{run.errorMessage}</p>
                )}
              </div>
            )}
          </div>
...
export function HistoryPage() {
  ...
  const PAGE_SIZE = 20
  const { page, setPage, totalPages, pageItems } = usePagination(filteredRuns, PAGE_SIZE)

  useEffect(() => { setPage(1) }, [search, statusFilter, sort, setPage])
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="shrink-0 border-b border-border px-6 py-5">
        <h1 className="text-xl font-semibold tracking-tight text-foreground">Pipeline History</h1>
        <p className="mt-0.5 text-sm text-muted-foreground">Browse previous AI pipeline executions.</p>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <table className="w-full min-w-215 text-sm">
          <thead className="sticky top-0 z-10 border-b border-border bg-background">
            <tr>
              {['Pipeline ID', 'Client', 'Started', 'Duration', 'Ideas', 'Approve', 'Consider', 'Reject', 'Status'].map(col => (
                <th
                  key={col}
          ...
          <tbody>
              ...
              <tr>
                <td colSpan={9} className="px-4 py-12 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <p className="text-sm text-muted-foreground">Failed to load pipeline history.</p>
              ...
              <tr>
                <td colSpan={9} className="px-4 py-16 text-center">
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm font-medium text-foreground">No pipeline executions yet</p>
                      <p className="text-xs text-muted-foreground">
            ...
            {!isLoading && !isError && runs.length > 0 && filteredRuns.length === 0 && (
              <tr>
                <td colSpan={9} className="px-4 py-12 text-center">
                  <p className="text-sm text-muted-foreground">No runs match your search or filter.</p>
                </td>
              </tr>
            )}

            {/* Data rows — fade in on mount */}
            {pageItems.map((run, i) => (
              <m.tr
                key={run.id}
                initial={{ opacity: 0, y: 4 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.18, delay: Math.min(i * 0.035, 0.28) }}
                className={cn(
      ...
      {/* Drawer — animated slide from right */}
      <AnimatePresence>
        {selectedRun && (
          <RunDrawer
            key={selectedRun.id}
